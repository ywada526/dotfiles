...
This repository manages personal dotfiles and local tool configuration, not an application codebase. Configs are grouped by tool into per-tool subdirectories (`shell/`, `git/`, `tmux/`, `vim/`, `mise/`, `codex/`, `claude/`); files inside keep their dot-prefixed destination names where applicable (e.g. `shell/.zshrc`, `git/.gitconfig`). Package-manager configs live flat under `pkg/`, one file per tool — generic filenames are prefixed with the tool name (e.g. `pkg/cargo-config.toml`, `pkg/go-env`) so the directory listing is self-documenting; the symlink target keeps the canonical name. The symlinks into `$HOME` and `~/.config` are declared in the `[dotfiles]` section of [`mise.toml`](/Users/ywada526/dotfiles/mise.toml); [`install.sh`](/Users/ywada526/dotfiles/install.sh) installs the prerequisites and then delegates to `mise bootstrap --only dotfiles`. [`shell/plugins.toml`](/Users/ywada526/dotfiles/shell/plugins.toml) defines Sheldon-managed Zsh plugins. [`mise.toml`](/Users/ywada526/dotfiles/mise.toml) at the repository root is the declarative machine bootstrap consumed by `mise bootstrap`; it is distinct from [`mise/.mise.toml`](/Users/ywada526/dotfiles/mise/.mise.toml), which is the global toolchain config symlinked to `~/.config/mise/config.toml`. [`setup.md`](/Users/ywada526/dotfiles/setup.md) documents post-install macOS setup and links into Dropbox-managed settings. [`README.md`](/Users/ywada526/dotfiles/README.md) stays intentionally minimal and only covers bootstrap installation.
...
- `./install.sh`: installs base dependencies, then applies the managed symlinks under `$HOME` and `~/.config` via `mise bootstrap --only dotfiles`.
- `mise dotfiles status`: report which managed symlinks are applied, missing, or point at a source that no longer exists.
...
- Bootstrap config: run `mise bootstrap status` to check parsing and drift, and `mise bootstrap --dry-run` to print the exact commands, before applying anything to the host machine.
