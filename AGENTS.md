...
This repository manages personal dotfiles and local tool configuration, not an application codebase. Configs are grouped by tool into per-tool subdirectories (`zsh/`, `git/`, `tmux/`, `vim/`, `mise/`, `codex/`, `claude/`, `security/`); files inside keep their dot-prefixed destination names (e.g. `zsh/.zshrc`, `git/.gitconfig`). Package-manager configs live flat under `pkg/`, one file per tool — generic filenames are prefixed with the tool name (e.g. `pkg/cargo-config.toml`, `pkg/go-env`) so the directory listing is self-documenting; the symlink target keeps the canonical name. [`install.sh`](/Users/ywada526/dotfiles/install.sh) symlinks each into `$HOME` or `~/.config`. [`zsh/plugins.toml`](/Users/ywada526/dotfiles/zsh/plugins.toml) defines Sheldon-managed Zsh plugins. [`setup.md`](/Users/ywada526/dotfiles/setup.md) documents post-install macOS setup and links into Dropbox-managed settings. [`README.md`](/Users/ywada526/dotfiles/README.md) stays intentionally minimal and only covers bootstrap installation.
